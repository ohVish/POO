...
Cadena::Cadena(unsigned n, char relleno) : s_(new char[n + 1]),
                                      tam_(n)
{
...
Cadena::Cadena(const Cadena &copia) : s_(new char[copia.tam_ + 1]),
                                      tam_(copia.tam_)
{
    strcpy(s_,copia.s_);
}

Cadena::Cadena(const char *caracteres) : s_(new char[strlen(caracteres) + 1]),
                                             tam_(strlen(caracteres))
...
Cadena Cadena::substr(int i, unsigned tam) const
{
    ...
    else
    {
        ...
        if (carposibles < tam)
            throw std::out_of_range{"Numero de caracteres se salen del límite."};
        else
        {
            Cadena c{tam};
